	...
	public void OnTriggerEnter(Collider col)
    {
                ...
                if (col.transform.parent.GetComponent<EnemyHealth>() != null)
                {
                    if (!Player.isRolling)
                    {
						...
						if ((Actions.Action == 1 || Actions.Action == 0) && CanHitAgain)
						{
							...
							newSpeed = new Vector3(Player.p_rigidbody.linearVelocity.x, 0, Player.p_rigidbody.linearVelocity.z);
							newSpeed.y = BouncingPower + Mathf.Abs (Player.p_rigidbody.linearVelocity.y);
							////Debug.Log (newSpeed);
							Player.p_rigidbody.linearVelocity = newSpeed;
						}
						...
						else if ((Actions.Action == 2 || Actions.PreviousAction == 2) && !StopOnHit && CanHitAgain)
                        {
							...
							newSpeed = new Vector3(Player.p_rigidbody.linearVelocity.x*(1/EnemyHomingStoppingPowerWhenAdditive), HomingBouncingPower, Player.p_rigidbody.linearVelocity.z*(1/EnemyHomingStoppingPowerWhenAdditive));
							////Debug.Log (newSpeed);
							Player.p_rigidbody.linearVelocity = newSpeed;
                        }
						else if(StopOnHit)
						{
							...
							newSpeed = Vector3.Scale(Player.p_rigidbody.linearVelocity, newSpeed);
        ...
        if (col.tag == "Spring")
        {
			...
			Actions.Action02.HomingAvailable = true;
                    ...
                    Player.p_rigidbody.linearVelocity += (spring.transform.up * spring.SpringForce);
                    Actions.ChangeAction(0);
		...
		if (col.tag == "Bumper")
		{
			...
			if (col.GetComponent<Spring_Proprieties>() != null)
			{
				...
				//	transform.position = col.transform.GetChild(0).position;
					...
					Player.p_rigidbody.linearVelocity += (Player.transform.position-spring.transform.position) * spring.SpringForce;
