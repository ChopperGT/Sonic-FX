...
public class Action00_Regular : MonoBehaviour {
    ...
    void FixedUpdate()
    {
		if(Player.SpeedMagnitude < 15 && Player.MoveInput == Vector3.zero && Player.Grounded)
		{
			...
			Player.p_rigidbody.linearVelocity *= 0.90f;
			hasSked = false;
		}
		...
		{
			if (Player.SpeedMagnitude >= -SkiddingIntensity) Player.AddVelocity(Player.p_rigidbody.linearVelocity.normalized * SkiddingIntensity * (Player.isRolling ? 0.5f : 1));
    ...
    void Update()
    {
        ...
        if (Player.Grounded)
        {
            Vector3 newForward = Player.p_rigidbody.linearVelocity - transform.up * Vector3.Dot(Player.p_rigidbody.linearVelocity, transform.up);
