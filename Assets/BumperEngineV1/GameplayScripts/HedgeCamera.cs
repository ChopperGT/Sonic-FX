...
public class HedgeCamera : MonoBehaviour
{
    ...
    void Awake()
    {
        // Load usable defaults even when the options menu has not initialized yet.
        SensiX = PlayerPrefs.GetFloat("X_SENS", 1f);
        SensiY = PlayerPrefs.GetFloat("Y_SENS", 1f);
        InvertedX = PlayerPrefs.GetInt("X_INV", 0) == 0 ? 1f : -1f;
        InvertedY = PlayerPrefs.GetInt("Y_INV", 0) == 0 ? 1f : -1f;
    }

    private Vector2 ReadCameraStick()
    {
#if ENABLE_INPUT_SYSTEM
        var gamepad = Gamepad.current;
        if (gamepad != null)
        {
            // Read only one input backend to avoid doubling the stick movement.
            Vector2 stick = gamepad.rightStick.ReadValue();
            // Match the vertical convention of the existing legacy camera axes.
    ...
    void CameraSet()
    {
        float y = Player.p_rigidbody.linearVelocity.y;
