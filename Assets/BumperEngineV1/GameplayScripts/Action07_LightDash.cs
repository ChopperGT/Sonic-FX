...
public class Action07_LightDash : MonoBehaviour {
	...
	void Update()
	{
		...
		//Set Animation Angle
		Vector3 VelocityMod = new Vector3(Player.p_rigidbody.linearVelocity.x, Player.p_rigidbody.linearVelocity.y, Player.p_rigidbody.linearVelocity.z);
    ...
    void FixedUpdate()
    {
		...
		if (Action.Action07Control.HasTarget) 
		{
			...
			Player.p_rigidbody.linearVelocity = direction.normalized * DashSpeed;
		...
		if (!Action.Action07Control.HasTarget)
		{
			...
			Player.p_rigidbody.linearVelocity = Vector3.zero;
			Player.p_rigidbody.linearVelocity = direction.normalized*EndingSpeedResult*EndingSpeedFactor;
