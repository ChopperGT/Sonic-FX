...
public class PlayerBhysics : MonoBehaviour
{
    ...
    void Update()
    {
        SpeedMagnitude = p_rigidbody.linearVelocity.magnitude;
        InputChecks();
    }
...
*/
    ...
    {
        //Stop Rolling
        if (p_rigidbody.linearVelocity.sqrMagnitude < 20)
        {
            isRolling = false;
        }
    ...
    void SlopePlysics()
    {
        ...
        if (p_rigidbody.linearVelocity.sqrMagnitude < SlopeSpeedLimit && SlopeRunningAngleLimit > GroundNormal.y)
        {
            transform.rotation = Quaternion.identity;
        ...
        {
            if (p_rigidbody.linearVelocity.y > StartDownhillMultiplier)
            {
                if (!isRolling)
                {
    ...
    public Vector3 NewStickToGround(Vector3 Velocity)
    {
        ...
        if (Grounded && TimeOnGround > 0.1f && SpeedMagnitude > 1)
        {
            ...
            Debug.DrawRay(Raycasterpos, p_rigidbody.linearVelocity * StickCastAhead * Time.deltaTime, Color.black, 1);
            if (Physics.Raycast(Raycasterpos, p_rigidbody.linearVelocity.normalized, out hitSticking, SpeedMagnitude * StickCastAhead * Time.deltaTime, Playermask))
            {
            ...
            else
            {
                if (Mathf.Abs(DirectionDot) < StickingNormalLimit) //avoid SuperSticking
                {
                    ...
                    if (Physics.Raycast(Raycasterpos + (p_rigidbody.linearVelocity * StickCastAhead * Time.deltaTime), -hit.normal, out hitSticking, 2.5f, Playermask))
                    {
                        float dist = hitSticking.distance;
                        if (EnableDebug)
                        {
                            Debug.Log("PlacedDown" + dist);
                            Debug.DrawRay(Raycasterpos + (p_rigidbody.linearVelocity * StickCastAhead * Time.deltaTime), -hit.normal * 3, Color.cyan, 2);
                        }
                        if (dist > 1.5f)
                        {
    ...
    {
        p_rigidbody.linearVelocity += force;
    }
    ...
    void AirMovement()
    {
        ...
        //Apply Gravity
        p_rigidbody.linearVelocity += Gravity;

        //Reduce speed
        if (MoveInput == Vector3.zero && StopAirMovementIfNoInput && p_rigidbody.linearVelocity.sqrMagnitude < 100)
        {
            Vector3 ReducedSpeed = p_rigidbody.linearVelocity;
    ...
    public void OnCollisionStay(Collision col)
    {
        ...
        foreach (ContactPoint contact in col.contacts)
        {
            ...
            if (p_rigidbody.linearVelocity.normalized != Vector3.zero)
            {
